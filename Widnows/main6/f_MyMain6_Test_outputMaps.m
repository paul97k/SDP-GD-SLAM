...
%------------------------------------1. measurment data:  Set Measurement ------------------
    ...
    % nominalX   = estimated_C_thetaFile.nominalX;
    % nominalXdepth  = estimated_C_thetaFile.nominalXdepth;
    ...
    nominalX = f_MyMain6_createNominalX(A,B,inputs, nT, x_0);
    ...
    % 
    % 
    nominalXdepth =            reshape(nominalX,       nx, nT)';
    % 
    nominalXdepth(:,1) = SDP_predicted_xy_norm(:,1);
    nominalXdepth(:,2) = SDP_predicted_xy_norm(:,2);
    nominalXdepth(:,3) = SDP_predicted_xy_norm(:,3);

    nominalXdepth = reshape(nominalXdepth', 1, nx*nT)';



    
    SDP_predicted_z_norm = estimated_C_thetaDepth * nominalXdepth;

    SDP_predicted_xy_norm(:,3)=SDP_predicted_xy_norm(:,3)-SDP_predicted_xy_norm(1,3);
    ...
    SDP_predictedStruct = struct( ...
        ...
        'depth',       SDP_predicted_z_norm-SDP_predicted_z_norm(1),...
    ...
    f_MyMain6_trimAndStructureData(SDP_predictedStruct, sonarStruct, measurementsStruct, groundTruthStruct, KalmanStruct);
...
% ----------------------------------Plot results-----------------------------------   
    ...
    f_MyMain6_plot_comparison( measurementsStruct, groundTruthStruct1, ...
        SDP_predictedStruct, KalmanStruct, OutputMapsDataFileName, ...
        TestDataFileName,DisturbanceLevel, itt, plot1, plot2)
