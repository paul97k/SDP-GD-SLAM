classdef classKalman < handle
    ...
    methods
        ...
        function obj = classKalman( identification)
        ...
        %     % 
        %     Input = [DataObj.CMD.cmd_linear_x  DataObj.CMD.cmd_linear_y DataObj.CMD.cmd_linear_z];
        ...
        % end
        % 
        function mapReturn = CreateKalmanMAP(obj, z_sonar, x_estimates)

            % DataObj = obj.DataObjGlobal;
            % 
            % z_sonar = DataObj.sonarPositions4.ranges0;
            
            % Initialize the map storage
            obj.map = [];

            t_sonar = linspace(0, 1, length(z_sonar));      % [0, 1] with 349 points
            t_xEst  = linspace(0, 1, length(x_estimates));      % [0, 1] with 105 points
            z_interp = interp1(t_sonar, z_sonar, t_xEst, 'linear');

            for i = 1:size(z_interp,2)
                z_i= z_interp(i);
                
                x_values = x_estimates(1, i) ;
                y_values = x_estimates(2, i);
                z_values = z_i;
            ...
            % 
            % % iFactor = size(z_sonar,1)/length(x_estimates);
            % iFactor = double(size(z_sonar,1)) / double(length(x_estimates));
            % 
            % if (1-abs(iFactor))<1
            %     iFactor=1;
            % end
            % 
            % % for i = 1:size(z_sonar,1)
            % for i = 1:size(z_sonar,2)
            %     % i/iFactor
            %     xEst_i = max([1,round(i/iFactor)]);
            %     z_i= z_sonar(i);
            % 
        ...
        function Z_est = CutAndGetNearestDataFromMap(obj, x_estimates, map_points)
        ...
        %           Z_est, Z_nearest, Z_GT] = CutAndGetNearestDataFromMap(obj, skip_i, skip_j, x_estimates, map_points)
            ...
            Z_est =     obj.findClosestZ(x_estimates(1,:),x_estimates(2,:), map_points);
