...
clc
close all
cd('C:\Users\paulk\iCloudDrive\Uni\Thesis\Codes\sasan code\main4')
...
AllSNR = [];
% parfor fileNumberTrain =1:length(trainFiles)
for fileNumberTrain =1:length(trainFiles)
    ...
    fileNumberTrain
        ...
        % disp("Use output map: " +DataFileName)
    if trainModel
        f_MyMain4_create_outputMaps(obj_ID, cutIdx, folderDataPath, dataFolder_OutputMapSDP, dataFolder_OutputMapSonarBased, DataFileName, controlInput, sigma)
        snr_dB = calculateSNR(obj_ID, cutIdx, folderDataPath, dataFolder_OutputMapSDP, dataFolder_OutputMapSonarBased, DataFileName, controlInput, sigma);
        AllSNR(fileNumberTrain) = snr_dB;
    else
        ...
        for testFilesI = 0:1
            ...
            % if ~contains(TestDataFileName, "Noise2")
            %     continue
            % end
            ...
            f_MyMain4_Test_outputMaps(obj_ID, cutIdx, SDPOutputMapFilePath, ...
                SonarOutputMapFilePath, folderDataPath, OutputMapsDataFileName, ... 
                TestDataFileName,  itt, controlInput, plot1,plot2, sigma)
...
function snr_dB =  calculateSNR(obj_ID, cutIdx, folderDataPath, dataFolder_OutputMapSDP, dataFolder_OutputMapSonarBased, DataFileName, trusterInput, sigma)
    ...
    noisySignal =  signal+ sigma* randn(size(signal));
