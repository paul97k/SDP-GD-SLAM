classdef classSDP_GD < handle
    ...
    methods
        function obj = classSDP_GD(A, B, dt, inputs, x_0, measurements, estimated_C_thetaFileMain4)
            ...
            obj.estimated_C_thetaFileMain4 = estimated_C_thetaFileMain4;
        end
        ...
        function obj = performSDP_GD(obj)
            ...
            nT = length(obj.measurements{1});
            ny = length(obj.measurements); 
            nx = 6;

            nominalX = obj.estimated_C_thetaFileMain4.nominalX;

            SDP_predicted_xy = obj.estimated_C_thetaFileMain4.estimated_C_theta*nominalX;

            nominalX =            reshape(nominalX,       nx, nT)';
            SDP_predicted_xy_norm =    reshape(SDP_predicted_xy, ny, nT)';
        
            nominalX(:,1) = SDP_predicted_xy_norm(:,1);
            nominalX(:,2) = SDP_predicted_xy_norm(:,2);
            nominalX(:,3) = SDP_predicted_xy_norm(:,3);
            % nominalX(:,1) = nominalX(:,1)*0;
            % nominalX(:,2) = nominalX(:,2)*0;
            % nominalX(:,3) = nominalX(:,3)*0;
            % nominalX(:,4) = nominalX(:,4)*0;
            % nominalX(:,5) = nominalX(:,5)*0;
            % nominalX(:,6) = nominalX(:,6)*0;
            ...
            % 
            % figure 
            % plot(obj.measurements{3}, '*')
            obj.nominalXdepth = reshape(nominalX', 1, nx*nT)';

            obj.estimated_C_thetaDepth = estimateCThetaDepth(obj, obj.measurements{3}, obj.nominalXdepth, obj.x_0);
            % hold on
            % plot(obj.estimated_C_thetaDepth*obj.nominalXdepth)
        ...
        function estimated_C_theta= estimateCThetaDepth(obj, measurment, nominalX ,x_0)
    ...
    % lifted_A, lifted_C, lifted_u, sigma_wx, sigma_v, sigma_w_theta, mu_theta
            ...
            [~, optimalVecVar, ~, ~, ~] = mapEstimateC(estimateBias, sigmaV, sigmaX, ...
